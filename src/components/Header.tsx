...
import { login, register, forgotPassword, resetPassword, search, getUserInfo, logout as logoutApi } from '../api/authApi';
import { LoginData, RegisterData, ResetPasswordData, SearchResult } from '@/types/auth';
...
const ForgotPasswordPopup = dynamic(() => import('./ForgotPasswordPopup'), { ssr: false });
const ResetPasswordPopup = dynamic(() => import('./ResetPasswordPopup'), { ssr: false });
...
const Header = () => {
    ...
    const [searchResults, setSearchResults] = useState<SearchResult>({ courses: [], posts: [] });
    ...
    */
    ...
    const handleSearch = useCallback(async (query: string) => {
        ...
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : 'Không thể tìm kiếm';
    ...
    const handleForgotPassword = async (email: string) => {
        ...
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : 'Email không tồn tại';
            toast.error(errorMessage);
        }
    };
    ...
    return (
        <header className="flex justify-between items-center px-6 py-3 bg-white shadow-md relative">
                        ...
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent className="w-80 p-2 shadow-lg rounded-2xl m-4">
                                ...
                                <DropdownMenuItem asChild>
                                    <Link href="/profile" className="block p-3 hover:bg-gray-100 rounded-lg cursor-pointer">
                                        Trang cá nhân
                                    </Link>
                                </DropdownMenuItem>
                                <DropdownMenuItem asChild>
                                    <Link href="/write-blog" className="block p-3 hover:bg-gray-100 rounded-lg cursor-pointer">
                                        Viết blog
                                    </Link>
                                </DropdownMenuItem>
                                <DropdownMenuItem asChild>
                                    <Link href="/my-posts" className="block p-3 hover:bg-gray-100 rounded-lg cursor-pointer">
                                        Bài viết của tôi
                                    </Link>
                                </DropdownMenuItem>
                                <DropdownMenuItem asChild>
                                    <Link href="/saved-posts" className="block p-3 hover:bg-gray-100 HEARTrounded-lg cursor-pointer">
                                        Bài viết đã lưu
                                    </Link>
                                </DropdownMenuItem>
                                <hr />
                                <DropdownMenuItem asChild>
                                    <Link href="/settings" className="block p-3 hover:bg-gray-100 rounded-lg cursor-pointer">
                                        Cài đặt
                                    </Link>
                                </DropdownMenuItem>
                                <DropdownMenuItem
            ...
            <ResetPasswordPopup
                ...
                onClose={closePopup}
                resetPasswordData={{ email: forgotEmail, resetCode: '', newPassword: '' }}
